...
- Automated ingestion pipeline from CSV files from S3 into PostgreSQL
...
1. The user exports financial transactions from the Money Flow mobile application in CSV format and uploads the file manually to S3-compatible object storage (Timeweb S3).

2. Prefect periodically runs the orchestration flow and checks S3 for available Money Flow CSV files.

3. The flow checks `infra.ingestion_file_registry` to determine whether the latest file has already been processed.

4. If the file is new, the Python ingestion layer loads it into `raw.money_flow` in PostgreSQL and writes ingestion metadata into `infra.ingestion_file_registry`.

5. If the file has already been processed, the flow exits without running downstream transformations.

6. After successful ingestion, Prefect triggers dbt transformations and tests.

7. dbt builds layered models:
   - `raw` - source-level ingested data
   - `stg` - technically normalized data from the latest successful ingestion batch
...
8. Observability models track ingestion freshness, dbt test results, model execution history and data quality metrics.

9. Superset dashboards consume dm and infra models for financial analytics and platform monitoring.

dbt does not access S3 directly and works only with data stored in PostgreSQL.


## Tech Stack

- PostgreSQL - primary analytical database and DWH storage
- dbt - SQL transformations, data modeling, testing, and documentation
- Python - data ingestion
- Prefect - workflow orchestration and pipeline scheduling
- Apache Superset - BI dashboards and data visualization
- Docker - containerization and runtime environment
- Timeweb S3 - S3-compatible object storage for source files
- Caddy - production reverse proxy and automated HTTPS
- GitHub Actions - continuous integration and automated project checks
- SQLFluff - dbt-aware SQL linting and style validation
...
```
├── .github/
│ └── workflows/            # GitHub Actions CI configuration and documentation
│
├── dbt/                    # dbt project: transformations, tests, seeds and macros
│ ├── analyses/             # reserved for ad-hoc analytical SQL queries
│ ├── macros/               # custom dbt macros
│ ├── models/               # layered DWH models: sources, stg, core, dm and infra
│ ├── seeds/                # static reference data
│ ├── snapshots/            # reserved for future historical tracking
│ └── tests/                # custom dbt data tests
│
├── docs/                   # project documentation
│ ├── architecture/         # architecture and end-to-end data flow
│ └── deployment/           # production deployment documentation
│
├── infra/                  # database bootstrap and deployment configuration
│ ├── bootstrap/            # roles, database settings, schemas, grants and base tables
│ └── deploy/               # Docker Compose, Superset, Prefect and Caddy configuration
│
├── ingestion/              # Python ingestion layer for loading Money Flow CSV files from S3
│
├── orchestration/          # Prefect orchestration layer
│ └── flows/                # Prefect flow definitions
│
├── tests/
│ └── ci/                   # synthetic data and CI-specific configuration
│
├── .pre-commit-config.yaml # local automated code quality checks
...
Expense and income charts are based on dates present in the transaction data rather than on a generated calendar spine.
When daily granularity is selected,
**days without financial operations are absent**
from the dataset instead of being represented as explicit zero values.
...
For more details, see `dbt/README.md`, `dbt/models/04_dm_demo/README.md`, `dbt/tests/demo/README.md`.
...
Workflow configuration and detailed CI documentation are stored under `.github/workflows/`
...
Git is required if the repository is cloned or updated directly on the host.
...
For development tools and direct execution outside Docker, the host system additionally requires:
...
```
git clone https://github.com/w3llnamed/finance_analytics.git
...
The `.env` file contains sensitive values, is excluded from version control and must not be committed.
...
Its `account_id` values must match the identifiers used in the private `dim_accounts.csv` file.
...
A host-level Python virtual environment is not required for Docker-based deployment.
...
Complete the [Environment Setup](#environment-setup) before starting the containers.
...
The `--build` option builds or rebuilds the custom Docker images before starting the containers.
During the Prefect Worker image build, Python project dependencies are installed from the repository `requirements.txt` file.
...
The same containerized services can be started locally or on a Linux server.
However, public server deployment additionally requires host-level configuration such as a firewall, DNS, a reverse proxy and HTTPS.
...
**The commands in the Quick Start section describe a local deployment**.
Public server deployment is configured separately at the operating-system and reverse-proxy level (`docs/deployment/vps.md`)
